...
export default function QuickPhoneContactWidget() {
  ...
  const packageTypes = [
    {
      ...
      helpText: "Include STAR Health Coordinator if assigned. Focus on mental health status.",
      required: true,
    },
    {
      value: "idd",
      label: "IDD/Autism Spectrum Disorder Support Services",
      helpText: "Education Portfolio required. Document behavioral supports.",
      required: true,
    },
    {
      value: "treatment",
      label: "T3C Treatment Foster Family Care Support Services",
      helpText: "Weekly contact schedule required. Document treatment progress.",
      required: true,
    },
    {
      value: "emergency",
      label: "Emergency Shelter Services",
      helpText: "Focus on immediate safety and stabilization.",
      required: true,
    },
    {
      value: "basic",
      label: "Basic Foster Care Services",
      helpText: "Standard foster care support and monitoring.",
      required: true,
    },
    {
      value: "therapeutic",
      label: "Therapeutic Foster Care Services",
      helpText: "Document therapeutic interventions and specialized care.",
      required: true,
    },
    {
      value: "kinship",
      label: "Kinship Care Services",
      helpText: "30-day pre-permanency planning. Focus on family stability.",
      required: true,
    },
    {
      value: "adoption",
    ...
    },
    {
      ...
      helpText: "6-month transition support. Alumni info mandatory.",
      required: true,
    },
    {
      value: "respite",
      label: "Respite Care Services",
      helpText: "Short-term support coordination.",
      required: true,
    },
    {
      value: "family-preservation",
      label: "Family Preservation Services",
      helpText: "Family strengthening strategies.",
      required: true,
    },
    {
      value: "reunification",
      label: "Family Reunification Services",
      helpText: "Transition planning and safety assessment.",
      required: true,
    },
    {
      value: "pregnant-parenting",
      label: "Pregnant/Parenting Youth Services",
      helpText: "Focus on dual-generation support.",
      required: true,
    },
    // Add-on services (optional)
    {
      value: "basic-foster-addon",
      label: "Basic Foster Home (Optional)",
      helpText: "Optional basic foster care support services.",
      required: false,
    },
    {
      value: "transition-addon",
      label: "Transition Support Services Add-On",
      helpText: "6-month planning required, alumni info mandatory.",
      required: false,
    },
    {
      value: "kinship-addon",
      label: "Kinship Caregiver Support Add-On",
      helpText: "30-day pre-permanency planning for kinship caregivers.",
      required: false,
    },
    {
      value: "pregnant-parenting-addon",
      label: "Pregnant & Parenting Youth Add-On",
      helpText: "Focus on dual-generation support services.",
      required: false,
    },
    {
      value: "other",
      label: "Other (please specify)",
      helpText: "Specify service type and requirements.",
      required: true,
    },
  ]

  // Base contact purposes
  const baseContactPurposes = [
  ...
  // Package-specific contact purposes
  const packageSpecificPurposes = {
    "transition-addon": [
      { value: "pal-checkin", label: "PAL Check-in" },
      { value: "alumni-connection", label: "Alumni Connection" },
    ],
    independent: [
      { value: "pal-checkin", label: "PAL Check-in" },
      { value: "alumni-connection", label: "Alumni Connection" },
    ...
    "pregnant-parenting": [
      { value: "parenting-support", label: "Parenting Support" },
      { value: "benefits-review", label: "Benefits Review" },
    ],
    "pregnant-parenting-addon": [
      { value: "parenting-support", label: "Parenting Support" },
      { value: "benefits-review", label: "Benefits Review" },
    ],
    kinship: [
  ...
  // Get available contact purposes based on selected package
  const getAvailableContactPurposes = () => {
    const packageSpecific = packageSpecificPurposes[contactData.packageType] || []
    return [...baseContactPurposes, ...packageSpecific]
  }
  ...
  useEffect(() => {
    const availablePurposes = getAvailableContactPurposes()
    if (!availablePurposes.find((p) => p.value === contactData.contactPurpose)) {
      setContactData((prev) => ({ ...prev, contactPurpose: "general" }))
    }
  ...
  const renderPackageSpecificChecks = () => {
    if (!contactData.packageType) return null

    const checks = []

    if (contactData.packageType === "pregnant-parenting" || contactData.packageType === "pregnant-parenting-addon") {
      checks.push(
        <label
          key="childWelfare"
          className="flex items-center p-2 bg-pink-50 rounded-lg cursor-pointer hover:bg-pink-100"
        >
          <input
            type="checkbox"
            name="childWelfareCheck"
            checked={contactData.childWelfareCheck}
            onChange={handleInputChange}
            className="mr-3 h-4 w-4 text-pink-500"
          />
          <span className="text-sm font-medium text-pink-800">Child welfare check completed?</span>
        </label>,
      )
    }

    if (contactData.packageType === "independent" || contactData.packageType === "transition-addon") {
      checks.push(
        <label
          key="transitionGoals"
        ...
        >
          <input
            ...
            checked={contactData.transitionGoalsProgress}
            onChange={handleInputChange}
            className="mr-3 h-4 w-4 text-blue-500"
          />
          <span className="text-sm font-medium text-blue-800">Progress on transition goals?</span>
        </label>,
      )
    }

    if (contactData.packageType === "kinship" || contactData.packageType === "kinship-addon") {
      checks.push(
        <label
          key="familyStability"
          className="flex items-center p-2 bg-green-50 rounded-lg cursor-pointer hover:bg-green-100"
        >
          <input
            type="checkbox"
            name="familyStabilityMaintained"
            checked={contactData.familyStabilityMaintained}
            onChange={handleInputChange}
            className="mr-3 h-4 w-4 text-green-500"
          />
          <span className="text-sm font-medium text-green-800">Family stability maintained?</span>
        </label>,
      )
    }
    ...
    return checks.length > 0 ? (
      <div className="space-y-2">
        <label className="block text-sm font-medium text-gray-700">Package-Specific Checks</label>
        {checks}
      </div>
    ) : null
  }
          ...
          <div className="p-4 space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center">
                Service Package
                <HelpTooltip text="Select package type for appropriate contact documentation requirements." />
              </label>
              <select
                name="packageType"
              ...
              {contactData.packageType && packageTypes.find((t) => t.value === contactData.packageType)?.helpText && (
            ...
            <div>
              ...
              >
                {getAvailableContactPurposes().map((purpose) => (
            ...
            {contactData.packageType && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                <h4 className="text-sm font-medium text-yellow-800 mb-1">Quick Reminders:</h4>
                <div className="text-xs text-yellow-700">
                  {contactData.packageType === "mental" &&
                    "• Ask about therapy attendance • Check medication compliance • Review crisis plan"}
                  {contactData.packageType === "idd" &&
                    "• Check educational progress • Review behavioral supports • Verify IEP compliance"}
                  {contactData.packageType === "treatment" &&
                    "• Document treatment progress • Check family dynamics • Review therapeutic goals"}
                  {(contactData.packageType === "kinship" || contactData.packageType === "kinship-addon") &&
                    "• Assess family stability • Review permanency planning • Check support services"}
                  {(contactData.packageType === "independent" || contactData.packageType === "transition-addon") &&
                    "• Check transition progress • Review life skills • Update alumni contact info"}
                  {(contactData.packageType === "pregnant-parenting" ||
                    contactData.packageType === "pregnant-parenting-addon") &&
                    "• Check parenting education • Review child development • Assess dual-generation needs"}
                </div>
              </div>
            )}
                      ...
                      <div className="flex-1">
                        ...
                        {contact.purpose && contact.purpose !== "general" && (
                          <span className="text-xs text-blue-600">
                            {getAvailableContactPurposes().find((p) => p.value === contact.purpose)?.label}
                          </span>
                        )}
                      </div>
